...
const ContEdit = () => {
  ...
  const addlivro = data => Api.put(`/livros/${id}`, data)
  .then(() => {
    console.log("foi");
    navigate("/");
  })
  ...
  const validationLiv = yup.object().shape({
    nome: yup.string().required("Está faltando o nome do livro.").max(50, 'Atingiu o nível máximo de caracteres'),
    autor:  yup.string().required("Está faltando o nome do autor.").max(50, 'passo de 20'),
    ...
    prefacio: yup.string().required("Está faltando o prefácio").max(300, 'passou de 300 caracteres')
  });
